...
frida_java_bridge_default.perform(function() {
  ...
  const OkHttpCall = frida_java_bridge_default.use("retrofit2.OkHttpCall");
  const Gson = frida_java_bridge_default.use("com.google.gson.Gson").$new();
  ...
  function toChapters(body) {
    ...
    try {
      ...
      return list.filter((c) => c && c.id && c.bookId).map((c) => ({
        id: c.id,
        bookId: c.bookId,
  ...
  const BaseObserver = frida_java_bridge_default.use("com.newreading.goodreels.net.BaseObserver");
  const NoopObserver = frida_java_bridge_default.registerClass({
    ...
    while (inFlight < MAX_IN_FLIGHT && queue.length > 0) {
      ...
      setImmediate(() => {
        try {
          ...
          k0.call(api2, chapterIds, task.bookId, noop);
        } catch (e) {
